...
echo nr32y = core.bm3dcuda_rtc.BM3D(pre_nr32y, sigma=[1.5, 0, 0]) >>"%~dp1%~nx1.vpy"
echo nr16y = mvf.Depth(nr32y, 16) >>"%~dp1%~nx1.vpy"

echo down = core.fmtc.resample(src16, src16.width/2, src16.height/2, sx=-0.5, css='444', planes=[3, 2, 2]) >>"%~dp1%~nx1.vpy"
echo RGB_uv = core.resize.Bicubic(down, format=vs.RGBS, matrix_in_s="709", filter_param_a_uv=0, filter_param_b_uv=0.5, range_in_s="limited") >>"%~dp1%~nx1.vpy"
echo OPP_uv = mvf.ToYUV(RGB_uv, matrix='OPP') >>"%~dp1%~nx1.vpy"
echo nr16uv = core.bm3dcuda_rtc.BM3D(OPP_uv, sigma=[1.0] * 3, chroma=True) >>"%~dp1%~nx1.vpy"
echo flt_uv = mvf.ToRGB(nr16uv, matrix='OPP') >>"%~dp1%~nx1.vpy"
echo nr16uv = core.resize.Bicubic(flt_uv, format=vs.YUV444P16, matrix_s="709", filter_param_a_uv=0, filter_param_b_uv=0.5, range_s="limited") >>"%~dp1%~nx1.vpy"
